<template>
  <div id="app">
    ...
    <h2>带 icon 的输入框</h2>
    <p>带有图标标记输入类型，可以通过 prefix-icon 和 suffix-icon 属性在 input 组件首部和尾部增加显示图标，也可以通过 slot 来放置图标。</p>
    ...
    <h2>带输入建议</h2>
    <p>根据输入内容提供对应的输入建议，autocomplete 是一个可带输入建议的输入框组件，fetch-suggestions 是一个返回输入建议的方法属性，如 querySearch(queryString, cb)，在该方法中你可以在你的输入建议数据准备好时通过 cb(data) 返回到 autocomplete 组件中。</p>
    ...
    <h2>自定义模板</h2>
    <p>可自定义输入建议的显示，使用scoped slot自定义输入建议的模板。该 scope 的参数为item，表示当前输入建议对象。</p>
